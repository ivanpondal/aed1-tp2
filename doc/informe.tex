...
\begin{ejercicio}
	\begin{problema}{top10}{g: Galeria}{result : [Imagen]}
		\asegura{
			(|imagenes(g)|<10) \rightarrow listaImagenesOrdenadasVotos(g, imagenes(g))
		}
		\asegura{
			(|imagenes(g)| \geq 10) \rightarrow listaImagenesOrdenadasVotos(g, imagenes(g))[0$..$10)
		}
	\end{problema}
\end{ejercicio}
...
\begin{ejercicio}
	...
	\begin{problema*}{agregarImagen}{g:Galeria, i: Imagen}
		...
		\asegura { i \notin imagenes(pre(g)) \rightarrow ( i \in imagenes(g) \land votos(g, i) == 0) }
	\end{problema*}
...
\begin{ejercicio}
      \begin{problema*}{votar}{g: Galeria, i: Imagen}
		...
		\asegura{ (\forall m \leftarrow imagenes(g), m \neq i)\ votos(pre(g), m) == votos(g, m)}
		\asegura{ votos(pre(g), i) + 1 == votos(g,i)}
      \end{problema*}
...
\begin{ejercicio}
	...
	\begin{problema*}{eliminarMasVotada}{g: Galeria}
		...
		\asegura{ (\forall i \leftarrow imagenes(g))\ votos(g, i) == votos(pre(g), i) }
	\end{problema*}
...
\begin{itemize}
	...
	\item \auxil{listaSumaCanalesPixeles(i: Imagen) : \TLista{\ent}}{ \newline
	  [sumaCanales \ | \ y \leftarrow [0..alto(i)), x \leftarrow [0..ancho(i)), sumaCanales==sumaCanalesPixel(color(i, x, y))]
	}
	...
	\item \auxil{ordenVotosDecreciente(g : Galeria) : [Imagen]}{ \newline
	  [i | (i \leftarrow imagenes(g), j \leftarrow [0.. |imagenes(g)|), k \leftarrow [0..|imagenes(g)|)), j < k \land votos(g,[j]) \geq votos(g, res[k]) \ ]
	}
